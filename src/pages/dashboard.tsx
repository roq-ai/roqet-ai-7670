...
import { getSalesActivities } from 'apiSdk/sales-activities';
...
import { getDefaultMessageVariables } from 'apiSdk/default-message-variables';
import { DefaultMessageVariablesInterface } from 'interfaces/default-message-variables';

import { SalesActivityListPage as ShowCaseList } from 'pages/sales-activities';

function DashboardPage() {
  const { session } = useSession();

  const salesActivityStat = useSWR<PaginatedInterface<SalesActivityInterface>>(
    () => `/sales-activities`,
    () => getSalesActivities({ limit: 1, offset: 0 }),
  );

  const messageTemplateStat = useSWR<PaginatedInterface<MessageTemplateInterface>>(
    () => `/message-templates`,
    () => getMessageTemplates({ limit: 1, offset: 0 }),
  );

  const defaultMessageVariablesStat = useSWR<PaginatedInterface<DefaultMessageVariablesInterface>>(
    () => `/default-message-variables`,
    () => getDefaultMessageVariables({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Sales Activities',
      result: salesActivityStat?.data?.totalCount,
      isLoading: salesActivityStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Message Templates',
      result: messageTemplateStat?.data?.totalCount,
      isLoading: messageTemplateStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Default Message Variables',
      result: defaultMessageVariablesStat?.data?.totalCount,
      isLoading: defaultMessageVariablesStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`sales-activities`} passHref legacyBehavior>
                  <Button
