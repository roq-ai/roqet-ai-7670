...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!session) {
    if (req.method === 'GET') {
      return getSalesActivitiesPublic();
    }
  ...
  async function getSalesActivitiesPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'sales_activity');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.sales_activity.count(countOptions as unknown),
      prisma.sales_activity.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function getSalesActivities() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.sales_activity
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'sales_activity'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
  ...
  async function createSalesActivity() {
    ...
    const data = await prisma.sales_activity.create({
      data: body,
    });
